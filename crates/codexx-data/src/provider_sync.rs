...
pub fn load_provider_sync_targets(codex_home: Option<&Path>) -> ProviderSyncTargetList {
    ...
    for db_path in codexx_core::codex_sqlite::codex_session_db_paths_from_home(&home) {
...
fn create_backup(
    ...
    for db_path in codexx_core::codex_sqlite::codex_session_db_paths_from_home(home) {
        for source in codexx_core::codex_sqlite::codex_sqlite_sidecar_paths(&db_path) {
            if !source.exists() {
                continue;
            }
            let relative = codexx_core::codex_sqlite::relative_to_codex_home(home, &source);
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("CodexX provider sync") {
            managed.push(path);
        }
    }
