...
pub async fn generate(request: StepwiseRequest, settings: &BackendSettings) -> anyhow::Result<Value> {
    if !settings.codex_app_stepwise_enabled {
        return Ok(json!({"status":"ok","disabled":true,"items":[]}));
    }
    let base_url = settings.codex_app_stepwise_base_url.trim().trim_end_matches('/');
    ...
    if base_url.is_empty() || model.is_empty() || api_key.is_empty() {
        return Ok(json!({"status":"failed","items":[],"error":"Stepwise Base URL、模型或 API Key 未配置"}));
    }
    ...
    let response = reqwest::Client::new()
        ...
        .headers(headers)
        .timeout(Duration::from_millis(settings.codex_app_stepwise_timeout_ms))
        .json(&json!({
            "model": model,
    ...
    if !status.is_success() {
        return Ok(json!({"status":"failed","items":[],"error":format!("上游 HTTP {}：{}", status.as_u16(), body.chars().take(240).collect::<String>())}));
    }
    let payload: Value = serde_json::from_str(&body).context("Stepwise 返回不是合法 JSON")?;
    Ok(json!({"status":"ok","items":extract_items(&payload, settings.codex_app_stepwise_max_items)}))
}

pub async fn test_connection(settings: &BackendSettings) -> anyhow::Result<Value> {
    generate(StepwiseRequest { last_user_message: "测试连接".to_string(), last_assistant_message: "请返回一条简短的下一步建议".to_string(), ..Default::default() }, settings).await
}
...
fn extract_items(payload: &Value, max_items: u8) -> Vec<StepwiseItem> {
    let candidates = [
        payload.get("choices").and_then(|v| v.get(0)).and_then(|v| v.get("message")).and_then(|v| v.get("content")),
        payload.get("output"), payload.get("result"), Some(payload),
    ];
    for candidate in candidates.into_iter().flatten() {
        let parsed = candidate.as_str().and_then(|text| serde_json::from_str::<Value>(text).ok()).unwrap_or_else(|| candidate.clone());
        let items = parsed.get("items").or_else(|| parsed.get("suggestions")).or_else(|| parsed.get("actions")).and_then(Value::as_array);
        if let Some(items) = items {
            let mut result = Vec::new();
            for item in items {
                let prompt = item.get("prompt").or_else(|| item.get("text")).or_else(|| item.get("action")).and_then(Value::as_str).unwrap_or("").trim();
                if prompt.is_empty() || result.iter().any(|existing: &StepwiseItem| existing.prompt == prompt) { continue; }
                result.push(StepwiseItem { label: item.get("label").or_else(|| item.get("title")).and_then(Value::as_str).unwrap_or("").trim().to_string(), prompt: shorten(prompt, 420) });
                if result.len() >= max_items as usize { break; }
            }
            return result;
        }
    }
...
fn stepwise_api_key(settings: &BackendSettings) -> String {
    ...
    if !direct.is_empty() { return direct.to_string(); }
    std::env::var(settings.codex_app_stepwise_api_key_env.trim()).unwrap_or_default().trim().to_string()
}
...
mod tests {
    ...
    fn extracts_and_limits_stepwise_items() {
        let items = extract_items(&json!({"items":[{"prompt":"A"},{"prompt":"A"},{"prompt":"B"}]}), 2);
