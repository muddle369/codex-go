use base64::Engine;
use codexx_core::assets;
use codexx_core::bridge::{self, BRIDGE_BINDING_NAME};
use codexx_core::cdp::{
...
fn injection_script_exposes_image_overlay_config() {
    ...
    .unwrap();
    let settings = codexx_core::settings::BackendSettings {
...
fn manager_ui_exposes_pure_api_relay_mode_button() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("core crate should live under crates/codexx-core");
    let source = std::fs::read_to_string(repo.join("apps/codexx-manager/src/App.tsx")).unwrap();
    let commands =
        std::fs::read_to_string(repo.join("apps/codexx-manager/src-tauri/src/lib.rs")).unwrap();
