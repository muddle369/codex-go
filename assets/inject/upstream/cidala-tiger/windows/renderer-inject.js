((cssText, artDataUrl, rawConfig) => {
  ...
  const ensure = () => {
    ...
    const modernHome = Boolean(home?.firstElementChild?.classList?.contains("px-toolbar") && home.children.length > 1);
    for (const candidate of document.querySelectorAll('[role="main"]')) {
      candidate.classList.toggle("dream-home", candidate === home && !modernHome);
