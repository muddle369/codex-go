...
use codexx_core::launcher::{
    DefaultLaunchHooks, LaunchHooks, LaunchOptions, launch_and_inject_with_hooks,
};
use codexx_core::models::{DeleteResult, ExportResult, SessionRef};
use codexx_core::routes::{BridgeContext, BridgeDataService, BridgeRuntimeService};
...
fn acquire_single_instance_guard(
    debug_port: u16,
) -> anyhow::Result<Option<codexx_core::ports::LoopbackPortGuard>> {
    ...
    match try_acquire_single_instance_guard() {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            ...
            if allow_stale_recovery && should_recover_stale_launcher(debug_port) {
                codexx_core::watcher::stop_launcher_processes();
        ...
        Err(error) => Err(error)
            .with_context(|| {
                format!(
                    ...
                    codexx_core::ports::LAUNCHER_GUARD_PORT
                )
            })
            .map(Some),
    }
...
fn try_acquire_single_instance_guard() -> std::io::Result<codexx_core::ports::LoopbackPortGuard> {
    codexx_core::ports::acquire_resilient_loopback_port_guard(
        codexx_core::ports::LAUNCHER_GUARD_PORT,
    )
}
...
fn log_launcher_guard_fallback(fallback_lock_path: &Path) {
    let _ = codexx_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "requested_guard_port": codexx_core::ports::LAUNCHER_GUARD_PORT,
...
fn should_recover_stale_launcher(debug_port: u16) -> bool {
    let has_codex_process = !codexx_core::watcher::find_codex_processes().is_empty();
    let cdp_listening = codexx_core::watcher::cdp_listening(debug_port);
    let recover =
        codexx_core::watcher::should_recover_stale_launcher(has_codex_process, cdp_listening);
    let _ = codexx_core::diagnostic_log::append_diagnostic_log(
...
async fn activate_existing_codex_app(options: &LaunchOptions) -> anyhow::Result<()> {
    ...
    let process_ids = codexx_core::watcher::find_codex_processes();
    let mut activated = false;
    #[cfg(windows)]
    {
        for process_id in &process_ids {
            if codexx_core::windows_activate_process_window(*process_id) {
                activated = true;
                break;
            }
        }
...
fn log_launcher_already_running(debug_port: u16) {
    let _ = codexx_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "guard_port": codexx_core::ports::LAUNCHER_GUARD_PORT,
            "debug_port": debug_port
        }),
    );
}
...
async fn notify_manager_when_update_available() -> anyhow::Result<bool> {
    let update = codexx_core::update::check_for_update(codexx_core::version::VERSION).await?;
...
impl LaunchHooks for LauncherHooks {
    fn resolve_app_dir(
        ...
        settings: &codexx_core::settings::BackendSettings,
    ...
    async fn load_settings(&self) -> anyhow::Result<codexx_core::settings::BackendSettings> {
        self.core.load_settings().await
    }

    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        let _ = tokio::task::spawn_blocking(|| codexx_data::run_provider_sync(None))
            .await
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<codexx_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &codexx_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &codexx_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: codexx_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    async fn delete(&self, session: SessionRef) -> anyhow::Result<DeleteResult> {
        ...
        let backup_store = codexx_data::BackupStore::new(self.backup_dir.clone());
        tokio::task::spawn_blocking(move || {
            codexx_data::delete_local_from_paths(db_paths, backup_store, &session)
        })
        .await
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let db_paths = self.candidate_db_paths();
        tokio::task::spawn_blocking(move || {
            codexx_data::export_markdown_from_paths(db_paths, &session)
        })
        .await
...
impl LauncherDataService {
    ...
    fn storage_adapter(&self) -> codexx_data::SQLiteStorageAdapter {
        codexx_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            codexx_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            codexx_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let target = codexx_core::cdp::pick_page_target(&targets)?;
        let url = codexx_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            std::process::Command::new(&manager_path)
                .creation_flags(codexx_core::windows_create_no_window())
                .spawn()
                .map_err(|error| anyhow::anyhow!("启动管理工具失败：{error}"))?;
        }
        #[cfg(not(windows))]
    ...
    async fn backend_status(&self) -> anyhow::Result<Value> {
        Ok(
            json!({"status": "ok", "message": "后端已连接", "version": codexx_core::version::VERSION}),
        )
    }
...
async fn try_inject_with_context(
    ...
    for target in injection_targets {
        ...
        let is_primary = target.id == primary_target.id;
        if is_primary {
...
fn pending_global_dictation_targets(
    ...
    targets
        ...
        .filter(|target| {
            codexx_core::cdp::is_injectable_page_target(target)
                && codexx_core::cdp::is_global_dictation_page_target(target)
                && !injected_target_ids.contains(&target.id)
        })
        .cloned()
        .collect()
}
...
async fn inject_pending_global_dictation_targets(
    ...
    let active_target_ids = targets
        .iter()
        .filter(|target| codexx_core::cdp::is_global_dictation_page_target(target))
...
async fn launcher_bridge_health_ok(debug_port: u16) -> anyhow::Result<bool> {
    let targets = codexx_core::cdp::list_targets(debug_port).await?;
    let primary = codexx_core::cdp::pick_injectable_codex_page_target(&targets)?;
    let websocket_url = primary
        .web_socket_debugger_url
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("selected CDP target has no websocket URL"))?;
    let result = codexx_core::bridge::evaluate_script_with_await_promise(
        websocket_url,
        codexx_core::bridge::bridge_health_check_script(),
        true,
    )
    .await?;
...
fn default_codex_db_path() -> PathBuf {
    codexx_core::codex_sqlite::codex_session_db_path()
}
...
fn manager_exe_path() -> PathBuf {
    ...
    dir.join(format!(
        "{}{}",
        codexx_core::install::MANAGER_BINARY,
        suffix
    ))
}
...
fn default_user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|dirs| dirs.home_dir().to_path_buf()) {
            return home.join("AppData").join("Roaming").join("CodexX");
        }
    }
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(codexx_core::install::MANAGER_BINARY))
        );
    }

    #[test]
    fn watchdog_discovers_global_dictation_target_created_after_initial_injection() {
        let targets = vec![
            codexx_core::cdp::CdpTarget {
                id: "primary".to_string(),
                target_type: "page".to_string(),
                title: "Codex".to_string(),
                url: "app://-/index.html".to_string(),
                web_socket_debugger_url: Some(
                    "ws://127.0.0.1:9329/devtools/page/primary".to_string(),
                ),
            },
            codexx_core::cdp::CdpTarget {
